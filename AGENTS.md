...
- **Cache media on disk, not in RAM.** Covers live in `%AppData%/Afterglow/media-cache`. Library and browse decode **thumbnails** (`DecodeToWidth`), not native 1080p/AVIF bitmaps. Hover galleries load on hover, not at refresh. Idle library RAM should stay in the hundreds of MB, not multiple GB.
- **GIFs are first-class.** A GIF cover or screenshot must play wherever it is shown (library/browse cards, detail, gallery). Keep original GIF bytes on disk; never treat a flattened PNG thumb as the GIF. Decode frames at the display width from the cached file (including hub `/api/v1/media/*.gif` URLs).
